/************************************************************
 *  parseInvoice.js
 *  Beispiel einer asynchronen Funktion zum Auslesen von
 *  Rechnungsdaten aus unstrukturiertem Text.
 ************************************************************/
 ...
 * Du kannst hier beliebig Vorverarbeitungsschritte ergänzen.
 */
function preprocessText(rawText) {
    // Beispiel: Zusammenführen von Zeilen, Entfernen doppelter Spaces etc.
    return rawText
  ...
  function parseInvoiceDate(text) {
    ...
    const dateLabels = [
      ...
      "datum:",
      "rechnung datum"
      // hier könnte man weitere ergänzen
    ];
  
    // Regex zum Erkennen von Datumsformaten wie:
    // - DD.MM.YYYY
    // - YYYY-MM-DD
    // - DD/MM/YYYY
    // - 16. Feb 2024,  etc.
    // - 21. August 2023, etc.
    // Wir vereinfachen hier stark. Du kannst natürlich noch mehr Formate abdecken.
    const dateRegex = new RegExp(
      ...
      "\\b(\\d{1,2}(\\.|-|\\/|\\s)?[A-Za-z]{3,9}|\\d{1,2}\\.\\d{1,2}|[A-Za-z]{3,9}\\.?\\s\\d{1,4})(\\.|-|\\/|\\s)?(\\d{4})\\b",
      "i"
    );
